...
$available_items = $foodItem->getAllItems();
...
<div id="searchResults" class="row" style="display:none;"></div>
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <!-- Search Bar -->
                <!-- <div class="mb-4">
                    <input type="text" id="searchBar" class="form-control" placeholder="🔍 Search for food items..." onkeyup="filterItems()">
                </div> -->
...
<?php foreach ($items as $item): ?>
    ...
    ?>
    <div class="col-md-6 mb-3 food-item-card">
        <div class="card h-100 <?php echo $is_available ? '' : 'bg-light text-muted'; ?>">
            <div class="card-body">
                <!-- Added class "item-name" for search -->
                <h5 class="card-title item-name"><?php echo $item['name']; ?></h5>
                <!-- Added class "item-desc" for search -->
                <p class="card-text item-desc"><?php echo $item['description']; ?></p>
                <p class="card-text">
                    <strong>Price: ₹<?php echo number_format($item['price'], 2); ?></strong>
                </p>
...
<?php endforeach; ?>
    ...
    const allItems = document.querySelectorAll(".food-item-card"); // each item card

    if (query === "") {
        // Empty query → show full category list again
        searchResults.style.display = "none";
        document.querySelectorAll(".category-section").forEach(section => section.style.display = "block");
        return;
    }

    // Hide category sections while searching
    document.querySelectorAll(".category-section").forEach(section => section.style.display = "none");

    // Filter items based on name (priority) or description
    let matchedItems = [];
    ...
    allItems.forEach(item => {
        ...
        // Prioritize food name — exact or partial match first
        if (name.startsWith(query) || name.includes(query)) {
    matchedItems.unshift(item);  // use original element
    ...
    if (matchedItems.length > 0) {
        matchedItems.forEach(item => {
            item.classList.add("col-md-6", "mb-3");
        ...
        searchResults.innerHTML = "<p class='text-muted'>No matching food items found.</p>";
