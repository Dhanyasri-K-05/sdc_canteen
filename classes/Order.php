...
class Order {
    ...
    //     $query = "INSERT INTO " . $this->table_name . " (user_id, bill_number, total_amount, payment_method) VALUES (?, ?, ?, ?)";
    ...
    $bill_number = 'BILL' . $today . $formatted_counter;

    // 🔹 Insert order record
    $query = "INSERT INTO " . $this->table_name . " (user_id, bill_number, total_amount, payment_method) 
              VALUES (?, ?, ?, ?)";
    $stmt = $this->conn->prepare($query);
    $stmt->execute([$user_id, $bill_number, $total_amount, $payment_method]);
