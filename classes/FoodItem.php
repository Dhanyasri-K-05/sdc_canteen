...
class FoodItem {
    ...
    public function requestAddItem($name, $description, $price, $quantity_available, $category, $cashier_id) {
    ...
    public function requestDeleteItem($item_id, $cashier_id) {
        ...
        $query = "INSERT INTO " . $this->requests_table . " (cashier_id, request_type, food_item_id, request_data) 
                  VALUES (?, 'delete_item', ?, ?)";
        $stmt = $this->conn->prepare($query);
    ...
    public function approveChange($request_id, $admin_id) {
        ...
        try {
            ...
            switch ($request['request_type']) {
                case 'add_item':
                    $query = "INSERT INTO " . $this->table_name . " (name, description, price, quantity_available, category) 
                              VALUES (?, ?, ?, ?, ?)";
                    $stmt = $this->conn->prepare($query);
                    $stmt->execute([
                        $request_data['name'],
